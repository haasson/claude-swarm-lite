...
// разницу ДО касания. «Правки без спроса» и «вообще без вопросов» — разные режимы живого
// Claude Code, и раньше одна кнопка обещала первое, а делала… тоже первое, хотя называлась
// автомодом. Теперь их две, и подписи не путаются.
test('кнопки режимов называют цену и не путаются между собой', () => {
  assert.match(T.QA_ACTIONS.edits, /правки без спроса/);
  assert.match(T.QA_ACTIONS.auto, /без вопросов/);
