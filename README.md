...
## Обычный терминал, только вкладок много

Внутри каждой вкладки — настоящий терминал с `claude`. Ваш шелл, ваши алиасы, ваши слэш-команды, ваша подписка. Всё, что вы делали в терминале, делается ровно так же: учить нечего.

Меняется одно. Когда агентов трое или четверо, вы перестаёте за ними следить — потому что **вкладка сама говорит, что с ней происходит**, а если вы нужны, приложение вас позовёт. Даже если вы вышли из дома.

Вот что для этого сделано:

- **Цвет вкладки = состояние агента.** Оранжевая работает, бирюзовая ждёт вашего ответа, зелёная закончила. Не надо открывать вкладку, чтобы понять, дошло ли дело.
- **Уведомление, когда вы нужны.** Агент упёрся в вопрос — прилетает системное уведомление, клик по нему открывает эту вкладку.
- **Пульт.** Отошли на полчаса, вернулись к четырём бирюзовым вкладкам — Пульт выстраивает их в очередь: ответили одному, он сам переключился на следующего.
- **Ответ с телефона.** Тема в Telegram на каждую вкладку. Пишете текст или голосовое, разрешения даёте кнопками.
- **Ветка и правки на виду.** Внизу окна — ветка папки активной вкладки, изменения и дифф по клику. Переключить ветку можно оттуда же.
- **Команды вашего проекта под рукой.** Приложение само находит слэш-команды и скиллы проекта и показывает их списком с поиском.
- **Видно, у кого кончается контекст.** На карточке вкладки — полоска заполнения, чтобы вовремя сжать разговор.
...
Клик по карточке открывает её терминал. Двойной клик по названию — переименовать. Крестик — закрыть вкладку вместе с процессом, с подтверждением. Карточки перетаскиваются за левый край.
...
Вкладка **Пульт** держит очередь всех, кто ждёт ответа, с таймером у каждого. Отвечаете в терминал прямо оттуда — очередь сама переходит к следующему.
...
Внизу окна — ветка папки активной вкладки и что с ней:
...
Полоска на карточке показывает, насколько заполнен контекст вкладки. Полная картина — команда `/usage` в Telegram: список всех вкладок, самые полные сверху, плюс окна подписки на 5 часов и на неделю со временем сброса.

Это ответ на вопрос «кого пора сжимать».

### Раскладка

Список вкладок бывает рейлом слева или плашками-дашбордом сверху. Переключается в настройках, там же предпросмотр.

<!-- ФОТО 9 · docs/img/09-layouts.png · две раскладки: рейл слева и дашборд сверху. -->

## Настроить под себя

Всё переключателями в самом приложении — кнопка **⚙ настройки**. Ни файлы править, ни команды набирать не нужно.
...
**Возвращаться в тот же разговор после перезапуска.** Папка и имя вкладки запоминаются всегда, диалог — по желанию.

**Чтобы вкладка отличала вопрос от завершения.** Клод заканчивает ход одинаково и когда сделал дело, и когда спросил. Галочка **«Просить агента звать вас»** включена по умолчанию и решает это сама — просто оставьте её включённой. Хотите свою формулировку — впишите её тут же, рядом есть поле для проверки.
...
1. **Бот.** `@BotFather` → `/newbot`, полученный токен вставить в первый шаг.
...
Под вторым шагом висит живой список требований — темы, бот в группе, админство, права. Каждый пункт с галочкой или крестиком: поправили в Telegram, нажали «Проверить ещё раз».
...
**Написать агенту.** Заходите в тему нужной вкладки и просто пишете — текст уходит в живую сессию, как будто вы набрали его на клавиатуре. Ни реплаев, ни команд не нужно: тема и есть адрес. Ответ придёт туда же, целиком и с нормальной разметкой.

<!-- ФОТО 14 · docs/img/14-tg-chat.png · телефон: диалог в теме вкладки — задача текстом и ответ агента. -->

**Дать разрешение.** Запрос приходит кнопками: в сообщении видно команду, которую одобряете, и пронумерованные варианты. Нажимаете номер.
...
**Показать картинку.** Скриншот в тему — и агент посмотрит его сам. Подпись становится задачей. Быстрее, чем пересказывать пальцами красную простыню из терминала.

**Навести порядок.** Переименовали вкладку на компьютере — переименуется тема, и наоборот. Тема заводится вместе со вкладкой и исчезает вместе с ней.
...
Уходя, переключите на телефон — иначе мост промолчит, а компьютер уснёт посреди хода. Забыли — скажите боту `/phone`, он сделает то же самое и досошлёт всё, что пропустили. Вернувшись, переключите обратно: пока выбран телефон, нижняя панель приложения подкрашена янтарным, чтобы это было видно сразу.

<!-- ФОТО 18 · docs/img/18-amber-bar.png · нижняя панель в янтарном режиме и переключатель положения. -->

Пока вы в телеге, агент отвечает по-телефонному: без стен кода и без интерактивного выбора вариантов, который с телефона нечем нажать. Насколько подробно — настройка «Кратко» или «Полностью».

## Горячие клавиши

Всё, что ниже, есть и кнопками — клавиши просто быстрее. На Windows вместо <kbd>⌘</kbd> — <kbd>Ctrl</kbd>.
...
Так и задумано. Приложение ставит вкладке свою строку — именно из неё берутся полоска контекста и ответ на `/usage`. Ваша строка остаётся при запуске Клода вне Swarm.
