...
**Основной способ — собрать у себя.** Нужны гит и нода. Приложение окажется в
«Программах» и откроется сразу, без блокировки и без снятия карантина:
...
**Если ноды нет — одной командой:**

```bash
curl -fsSL https://raw.githubusercontent.com/raul-cortez/claude-swarm/main/scripts/install.sh | sh
```

Блокировка «не удалось проверить разработчика» достаётся файлам, помеченным карантином,
а помечает их браузер, которым качали. Ни у собранного у себя, ни у принесённого `curl`
такой пометки нет — отсюда и разница.

**Если качал браузером.** Открой `.dmg`, перетащи **Swarm** в **Applications** и сними
карантин — это делается один раз:
