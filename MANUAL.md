...
**Проще всего — одной командой.** Приложение окажется в «Программах» и откроется сразу,
без блокировки и без снятия карантина:
...
а помечает их браузер, которым качали. `curl` не помечает — отсюда и разница.
...
**Если стоит нода**, можно собрать у себя: `npm ci && npm run install-app`. Карантина у
собранного тоже нет.
