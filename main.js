...
//
//   desk  «за компом»    — обычная работа: в группу только вопросы и разрешения. Иначе
//                          телефон жужжал бы весь рабочий день над каждым ходом.
//   phone «за телефоном» — всё разом: итоги ВСЕХ ходов уезжают в группу, агентов просим
//                          отвечать кратко (читать простыню с телефона нечем), и мак не
//                          засыпает — иначе отвечать некому, агенты живут здесь.
//
...
// Уезжают ли в группу итоги ВСЕХ ходов. Два источника, и они складываются: «где я» на
// сегодня и галка «слать итоги всех вкладок» навсегда.
function tgMirrors() {
...
// «За телефоном» краткость включается сама, что бы ни стояло в настройке: с телефона
// читают одним глазом, и простыня там не ответ, а препятствие. Настройку при этом НЕ
// трогаем — вернулся за стол, и «полностью» снова в силе, ничего восстанавливать не надо.
function tgPromptDefault() {
  return telegram.detailPrompt(tgPresence === 'phone' ? 'short' : TG.detail);
}
...
function tgOnUpdate(u) {
  ...
  if (u.command === 'start' || u.command === 'help') {
    tgSend({ threadId: u.threadId, text: [
      ...
      '/phone — я с телефоном: слать сюда все итоги, отвечать кратко, маку не спать',
      '/comp — я вернулся за компьютер: сюда только вопросы',
...
async function tgWhereAmI(u, presence) {
  ...
  const what = phone
    ? 'Итоги всех ходов идут сюда, агентов прошу отвечать кратко, маку спать не даю.'
      + (TG.keepAwake ? '' : ' Сон, правда, выключен галкой в настройках — там его и включать.')
    : 'Сюда идут только вопросы и запросы разрешений.'
      + (TG.mirrorAll ? ' Итоги всё равно идут все: включена галка «присылать итоги всех ходов всегда».' : '');
...
const TG_PRESENCE_SAID = {
  ...
  phone: 'за телефоном — итоги всех ходов в группу, просим отвечать кратко, мак не засыпает',
};
...
function tgSetPresence(raw, from) {
  ...
  tgApplyPrompt();      // «за телефоном» просим агентов отвечать кратко
  tgApplyKeepAwake();
